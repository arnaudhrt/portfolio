...
export default function ProjectDialog({ name }: { name: string }) {
  ...
  const handleCick = () => {
    if (project.linkRepo === "private") {
      ...
      window.open(project.linkRepo, "_blank");
    }
  };
  return (
    <div>
        ...
        <DialogHeader>
          <div className="flex gap-4 items-center mb-3">
            <div className="rounded-full p-2 size-14 bg-accent flex justify-center items-center">
              <img src={project.logo} alt={project.name} width={100} height={100} className="" />
            </div>
            <div>
          ...
          <div className="space-y-4">
            ...
            <div className="flex gap-6 items-center">
              ...
              <div className="flex gap-1 items-center group">
                <IoLink className="size-5 text-muted-foreground group-hover:text-foreground" />
                <a href={project.LinkApp} target="_blank" rel="noopener noreferrer" className="text-sm text-muted-foreground group-hover:text-foreground">
                  Live App
                </a>
              </div>
            </div>
